...
open class Countries {
    ...
    open static let isSelected = UserDefaults.standard.bool(forKey: "countryIsSelected")
    ...
    open static func saveSelectedCountry(_ country: Country) {
        UserDefaults.standard.set(country.name, forKey: selectedCountryName)
        if country.phoneExtension != nil {
            UserDefaults.standard.set(country.phoneExtension!, forKey: "selectedCountryPhoneExtension")
        }
        if country.isMain != nil {
            UserDefaults.standard.set(country.isMain!, forKey: selectedCountryisMain)
        }
        UserDefaults.standard.set(country.countryCode, forKey: selectedCountryCode)
        UserDefaults.standard.set(true, forKey: countryIsSelected)
        UserDefaults.standard.synchronize()
    }
    
    open static func getSelectedCountry() -> Country?  {
        let isSelected = UserDefaults.standard.bool(forKey: countryIsSelected)
        if isSelected {
            let countryName = UserDefaults.standard.object(forKey: selectedCountryName) as! String
            let countryCode = UserDefaults.standard.object(forKey: selectedCountryCode) as! String
            
            guard let phoneExtension = UserDefaults.standard.object(forKey: selectedCountryPhoneExtension) as? String else {
                let country = Country(name: countryName, phoneExtension: nil, countryCode: countryCode, isMain: nil)
                return country
            }
            guard let isMain = UserDefaults.standard.object(forKey: selectedCountryisMain) as? Bool else {
                let country = Country(name: countryName, phoneExtension: nil, countryCode: countryCode, isMain: nil)
                return country
            }
